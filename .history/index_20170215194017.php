...
//Config::$data['default']['database']['type'] = 'sqlite';
    ...
    **/
    ...
    Config::$data['layout_data'] = ROOT.STORE."layouts.json";
    Config::$data['menu_data'] = ROOT.STORE."menus.json";
    
    if(Helper::Get('layout-mnglayouts')=="xxx"){
        Loader::show_module(SYS.C.'layout'.S.'mnglayouts','');
    } elseif(Helper::Get('load')=="admin") {
        Loader::show_module(SYS.C.'admin'.S.'mngmenus',SYS.V.'admin'.S.'choose');
    } else {
        Loader::show_module(SYS.C.'template'.S.'template',SYS.THEMES.Config::$data['template']['any'].DS.'theme');
    }
