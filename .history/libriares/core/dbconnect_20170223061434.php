...
class DBConnect {
    ...
    public function isLock($name){
        ...
        if ($data['type']=='sqlsrv') {
            ...
            $sql="SHOW OPEN TABLES  WHERE `Table` LIKE '%".$name."%' AND  `Database` LIKE '%".$data['name']."%' AND In_use > 0 OR Name_locked > 0;";
        } elseif ($data['type']=='sqlite') {
    ...
    public function createTable($table,$columns) {
        ...
        if ($data['type']=='sqlsrv') {
            ...
            $sql="CREATE SEQUENCE ".$table."_id_seq;".
    ...
    public function Delete($table,$sql) {
        $del = $this -> db -> query('DELETE FROM '.$table.' WHERE '.$sql);
        $check = $del->rowCount();
        if ($check > 0) {
            return $check;
        } else {
    ...
    public function Insert($table,$data) {
        try {
            ...
            $add = $this -> db -> prepare("INSERT INTO ".$table." (".implode(",", $keys).") VALUES (".substr($string,0,strlen($string)-1).")");
            $add -> execute($values);
            $chk = $add -> rowCount();
            if ($chk > 0) {
                return $chk;
    ...
    public function Update($table,$data,$query='') { // ".$query[]."
        try {
            $chk = 0;
            $keys = array();
            $values = array();
    ...
    public function preBegin($name='T1'){
        $data=Config::$data['default']['database'];
        if ($data['type']=='sqlsrv') {
            $sql="BEGIN TRAN $name;";
        } elseif ($data['type']=='pgsql') {
    ...
    public function preCommit($name='T1'){
        $data=Config::$data['default']['database'];
        if ($data['type']=='sqlsrv') {
            $sql="COMMIT TRAN $name;";
        } elseif ($data['type']=='pgsql') {
    ...
    public function preRelease($name='T1'){
        $data=Config::$data['default']['database'];
        if ($data['type']=='sqlsrv') {
            $sql="";
        } elseif ($data['type']=='pgsql') {
    ...
    public function preRollback($name='T1'){
        $data=Config::$data['default']['database'];
        if ($data['type']=='sqlsrv') {
            $sql="ROLLBACK TRAN $name;";
        } elseif ($data['type']=='pgsql') {
    ...
    public function trDelete($table,$sql) {
        $del = $this -> db -> query('DELETE FROM '.$table.' WHERE '.$sql);
        $check = $del->rowCount();
        if ($check > 0) {
            return $check;
    ...
    public function trInsert($table,$data) {
        try {
            $chk = 0;
            $keys = array();
            $values = array();
    ...
    public function trUpdate($table,$data,$query='') { // ".$query[]."
        try {
            $chk = 0;
            $keys = array();
            $values = array();
    ...
    public function search_entries($table,$gprx,$limit=100,$offset=0) {
        $max = $offset+$limit;
        $h = $this -> db -> prepare("SELECT * FROM ".$table." WHERE gprx=? AND idx>=? AND idx<? ORDER BY idx ASC");
    ...
    public function search_name_value($table,$name,$value,$gprx,$limit=100,$offset=0) {
        $max = $offset+$limit;
        $h = $this -> db -> prepare("SELECT * FROM ".$table." WHERE name=? AND value=? AND gprx=? AND idx>=? AND idx<? ORDER BY idx ASC");
    ...
    public function query_value($table,$name,$value,$gprx,$limit=100,$offset=0) {
        $max = $offset+$limit;
        $h = $this -> db -> prepare("SELECT * FROM ".$table." WHERE name=? AND value LIKE ? AND gprx=? AND idx>=? AND idx<? ORDER BY idx ASC");
        $h -> execute(array($name,'%'.$value.'%',$gprx,$offset,$max));
        $rows = $h -> fetchAll(PDO::FETCH_NAMED);
        if ($rows) {
            //return $rows;
    ...
    public function search_name($table,$name,$gprx,$limit=100,$offset=0) {
        $max = $offset+$limit;
        $h = $this -> db -> prepare("SELECT * FROM ".$table." WHERE name=? AND gprx=? AND idx>=? AND idx<? ORDER BY idx ASC");
    ...
    public function search_name_idx($table,$name,$idx,$gprx) {
        $h = $this -> db -> prepare("SELECT * FROM ".$table." WHERE name=? AND idx=? AND gprx=? ORDER BY idx ASC");
    ...
    public function search_idx_enteries($table,$idx,$gprx) {
        $h = $this -> db -> prepare("SELECT * FROM ".$table." WHERE idx=? AND gprx=? ORDER BY idx ASC");
...
// get
    ...
    public function get_name_id($table,$name,$idx,$gprx) {
        $h = $this -> db -> prepare("SELECT id FROM ".$table." WHERE name=? AND idx=? AND gprx=? ORDER BY id ASC");
        $h -> execute(array($name,$idx,$gprx));
        $rows = $h -> fetchAll(PDO::FETCH_NAMED);
        if ($rows) {
            //$this->sortby($rows,'pos');
    ...
    public function get_gprx_list($table) {
        $h = $this -> db -> prepare("SELECT gprx FROM ".$table." ORDER BY gprx ASC");
        $h -> execute(array());
    ...
    public function get_idx_list($table,$gprx,$limit=100,$offset=0) {
        $max = $offset+$limit;
        $h = $this -> db -> prepare("SELECT idx,gprx FROM ".$table." WHERE gprx=? AND idx>=? AND idx<? ORDER BY idx ASC");
        $h -> execute(array($gprx,$offset,$max));
        $grp = $h -> fetchAll(PDO::FETCH_NAMED);
        if ($grp) {
            $tmp = array();
    ...
    public function delete_item($table,$name,$idx,$gprx) {
        $del = $this -> db -> prepare('DELETE FROM '.$table.' WHERE name=? AND  idx=? AND gprx=?');
    ...
    public function delete_idx($table,$idx,$gprx) {
        $del = $this -> db -> prepare('DELETE FROM '.$table.' WHERE idx=? AND gprx=?');
    ...
    public function add_item($table,$name,$value,$idx,$gprx) {
        try {
            $a = $this -> db -> query("SELECT name, value, idx, gprx FROM ".$table." WHERE name='$name' AND idx='$idx' AND gprx='$gprx' ");
            $check = $a -> fetchColumn();
            if ($check == TRUE) {
                return 1069;
            } else {
                $add = $this -> db -> prepare("INSERT INTO ".$table." (name, value, idx, gprx) VALUES (?,?,?,?)");
    ...
    public function update_item($table,$name,$value,$idx,$gprx) {
        try {
            $a = $this -> db -> query("SELECT * FROM ".$table." WHERE name='$name' AND idx='$idx' AND gprx='$gprx' ");
            $check = $a -> fetchColumn();
            if ($check != FALSE) {
                $add = $this -> db -> prepare("UPDATE ".$table." SET name=?,value=?,idx=?,gprx=? WHERE idx=? AND gprx=?");
