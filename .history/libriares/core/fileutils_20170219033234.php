	...
	while (count($dir) != 0) {
		...
		foreach (glob($v) as $item) {
			if (is_dir($item)) {
				...
				$info[dirname(realpath($item))][0] = array('path'=>realpath($item),'name'=>basename($file),'type'=>$type,'owner'=>$owner[0],'group'=>$owner[1],'perms'=>$perms);
			}
			else {
				$type = filetype(realpath($item));
				$owner = self::getOwner(realpath($item));
				$perms = self::getPerms(realpath($item));
				$info[dirname(realpath($item))][] = array('path'=>realpath($item),'name'=>basename($file),'type'=>$type,'owner'=>$owner[0],'group'=>$owner[1],'perms'=>$perms);
			}
		}
	}
	return $info;
