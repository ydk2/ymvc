...
class Cache
{
	...
	public function setCacheString($cache){
        if(!empty($cache)){
	...
	public function setCache($settings,$cache){
        if(!empty($cache)){
    ...
    public function ItemExists($items,$item,$by='id'){
		$chk = 0;
		reset($items);
	...
	public function freekey($cache,$by='id'){
