...
class DBConnect {
    ...
    public function Insert($table,$data) {
        try {
            ...
            $add = $this -> db -> prepare("INSERT INTO ".$table." (".implode(",", $keys).") VALUES (".substr($sign,-2).")");
            $add -> execute(array($values));
            $chk = $add -> rowCount();
            if ($chk > 0) {
                return TRUE;
            }
    ...
    public function Update($table,$data,$query='') { // ".$query[]."
        try {
            ...
            $add = $this -> db -> prepare("UPDATE ".$table." SET ".substr($string,-2)." ".$query);
            $add -> execute(array($values));
            $chk = $add -> rowCount();
            if ($chk > 0) {
                return TRUE;
