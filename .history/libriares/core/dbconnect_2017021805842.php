...
class DBConnect {
    ...
    public function preDelete($table,$sql) {
        return 'DELETE FROM '.$table.' WHERE '.$sql.';';
    }

    public function preInsert($table,$data) {
            $keys = array();
            $values = array();
            $string='';
            reset($data);
            ...
            return $this -> db -> prepare("INSERT INTO ".$table." (".implode(",", $keys).") VALUES (".substr($string,0,strlen($string)-1).");");

    }

    public function preUpdate($table,$data,$query='') {
            $chk = 0;
            $keys = array();
            $values = array();
            $string='';
            ...
            return "UPDATE ".$table." SET ".substr($string,0,strlen($string)-1)." ".$query.";";
    }
