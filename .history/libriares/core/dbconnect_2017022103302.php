...
class DBConnect {
    ...
    public function query_value($table,$name,$value,$gprx,$limit=100,$offset=0) {
        $max = $offset+$limit;
        $h = $this -> db -> prepare("SELECT * FROM ".$table." WHERE name=? AND value LIKE %".$value."% AND gprx=? AND idx>=? AND idx<? ORDER BY idx ASC");
...
// get
    ...
    public function get_idx_list($table,$gprx) {
        $h = $this -> db -> prepare("SELECT idx,gprx FROM ".$table." WHERE gprx=? ORDER BY idx ASC");
