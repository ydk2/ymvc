	...
	public static function inDir($path = './*') {
		...
		foreach (glob("$path",GLOB_BRACE) as $file) {
			if (basename($file) != '..' && basename($file) != '.') {
				...
				$info[basename($file)] = array('dir'=>is_dir(realpath($file)),'path'=>realpath($file),'name'=>basename($file),'type'=>$type,'owner'=>$owner[0],'group'=>$owner[1],'perms'=>$perms);
			}
		}
		return $info;
	}
	...
	while (count($dir) != 0) {
		...
		foreach (glob($v) as $item) {
			if (is_dir($item)) {
				...
				$info[dirname(realpath($item))][0] = array('dir'=>true,'path'=>realpath($item),'name'=>basename($file),'type'=>$type,'owner'=>$owner[0],'group'=>$owner[1],'perms'=>$perms);
			}
			else {
				$type = filetype(realpath($item));
				$owner = self::getOwner(realpath($item));
				$perms = self::getPerms(realpath($item));
				$info[dirname(realpath($item))][] = array('dir'=>false,'path'=>realpath($item),'name'=>basename($file),'type'=>$type,'owner'=>$owner[0],'group'=>$owner[1],'perms'=>$perms);
			}
		}
	}
	return $info;
