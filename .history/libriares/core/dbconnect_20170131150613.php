...
// DELETE FROM layouts WHERE idx=10 AND gprx=layout
    ...
    public function doSave($item='item',$table,$group){
        if(Helper::get('action')=='add' && isset($_POST['add']) && isset($_POST[$item])){
        ...
        if(Helper::get('action')=='update' && isset($_POST['update']) && isset($_POST[$item])){
            $frompost = Helper::post($item);
            reset($frompost);
            $chk = 0;
            $data = $this->reverseNoId($frompost,$group);
        ...
        if(Helper::get('action')=='delete' && Helper::get($item)){
			$out=$this->delete_idx($table,Helper::get($item),$group);
                if($chk == 0){
                    return TRUE;
                } else {
                    return FALSE;
