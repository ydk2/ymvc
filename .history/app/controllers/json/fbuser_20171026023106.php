...
class FBUser extends \library\Core\Controller
{
  ...
  protected function isUser()
  {
    ...
    $ud = array('account_email', 'id');$scope = ['id', 'account_login', 'account_email', 'account_born', 'account_role'];
    $this->ViewData('scope', json_encode($scope));

    $chk = $db->TSelect('accounts', $ud, "WHERE account_email=?", [$email]);
    if ($chk) {
      ...
      if ($auth->error != 200) {
        $auth->authorizeid($chk[0]['id']);
        $this->ViewData('access_token', $auth->access_token);
