...
class Main extends \library\Core\Controller
{
    public function __construct($model)
    {
        ...
        $this->ViewData('header','Main View');
        //header('Location: '.HOST.'?path=login');
        //$this->test();
        //$this->error = $this->auth->error;
        //Helper::CookieRegister('access_token',$this->auth->access_token,60*60*24*365);
        if (!$this->model->isAjax()) {
            if($this->auth->error != 200){
                header('Location: '.HOST.'?path=login');
                exit();
            } else {
              //  Helper::CookieRegister('access_token',$this->auth->access_token,60);
            }
        }
    ...
    //Inc('test/views/test/view','.html');  
       ...
       //echo "SID: ".SID."<br>session_id(): ".session_id()."<br>COOKIE: ".$_COOKIE["PHPSESSID"];
        ...
        /* *
        $this->auth = ['*'];
        $access_token = Helper::Get('access_token');
        $appid = 'bbj377hnm6sn49i998jrgbr33m';
        list($user_id, $access_token) = explode(',', base64_decode($access_token));
        $check_token = $db->TSelect('accounts_token', $this->auth, 'WHERE client_id=? AND user_id=? AND access_token=?', [$appid, $user_id, $access_token]);
