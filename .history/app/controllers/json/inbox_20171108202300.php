...
class Inbox extends \library\Core\Controller
{
    ...
    private function update()
    {
        ...
        $msg = $this->db->TSelect('accounts_msg', ['*'], " WHERE msgid NOT IN (SELECT msgid,for_id,from_id FROM accounts_msg WHERE for_id=? AND from_id<>?) AND for_id<>? AND from_id<>? AND status=0 ORDER BY mtime DESC", [$for_id, $from_id,$for_id, $from_id]);
