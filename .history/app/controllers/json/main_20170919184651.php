...
class Main extends \library\Core\Controller
{
    ...
    //Inc('test/views/test/view','.html');  
        ...
        */
        ...
        $auth = ['*'];
        $check_token = $db->TSelect('accounts_token', $auth, 'WHERE client_id=? AND access_token=?', [$sessionid,Helper::Get('access_token')]);
        if ($check_token) {
            //$token = bin2hex(openssl_random_pseudo_bytes(32));

            $token = $check_token[0]['access_token'];
            $scope = json_decode($check_token[0]['scope'],TRUE);
            $user = $db->TSelect('accounts', $scope, 'WHERE ' . $scope[0] . '=?', [$check_token[0][$scope[0]]]);
