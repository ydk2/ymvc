...
class Login extends \library\Core\Controller
{
    ...
    //Inc('test/views/test/view','.html');  
        ...
        */
        ...
        $this->ViewData('token', '?access_token=' . $auth->token);
...
class lAuth
{
    ...
    public function login()
    {
        $token = "";
        $request = FALSE;
        $user = $db->TSelect('accounts', $this->scope, 'WHERE account_login=? OR account_email=? AND account_pass=?', [Helper::Post('account_login'), Helper::Post('account_login'), hash('sha256', Helper::Post('account_pass'))]);
 
        if ($user && isset($user[0])) {
            $this->userid = $user[0]['id'];
            $auth = ['*'];
            $expires = time() - 60;
            $check_token = $db->TSelect('accounts_token', $auth, 'WHERE user_id=? AND client_id=?', [$this->userid, $this->appid]);
            if ($check_token) {
                $expires = $check_token[0]['expires'];
            }
            if (!$check_token || (strtotime($expires) < time())) {
