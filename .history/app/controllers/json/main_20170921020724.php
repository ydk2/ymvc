...
class Main extends \library\Core\Controller
{
    ...
    //Inc('test/views/test/view','.html');  
        ...
        */
        ...
        $auth->request();
        //$request = $check_token;
        $this->ViewData('request', $auth->decode(sha1($appid),$auth->encode(sha1($appid),json_encode($auth->request))));
        ...
        $this->ViewData('response', $auth->decode(sha1($appid),$auth->encode(sha1($appid),'<a href="?logout=true">logout</a>')));
