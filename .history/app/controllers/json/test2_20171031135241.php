...
class Test2 extends \library\Core\Controller
{
    ...
    public function Run()
    {
        ...
        * */
        ...
        $now = date("Y-m-d H:i:s", time());
        $redirect = HOST.'';
        $this->ViewData('data', '{"error":'.$error.',"time":"'.$now.'","expires":"'.$now.'"}');
        header('Last-Modified: '.gmdate('D, d M Y H:i:s', filemtime(__FILE__)).' GMT', true, $error);
        header("Location: ".$redirect,TRUE,$error);
    }
