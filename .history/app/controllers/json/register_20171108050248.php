...
class Register extends \library\Core\Controller
{
    ...
    //Inc('test/views/test/view','.html');  
        ...
        $this->ViewData('request', json_encode($_POST));

        $step = intval(Helper::Get('step'));
        if($step == 1){
            $data = [
                'account_ctime'=>time(),
                'account_login'=>Helper::Post('account_email'),
                'account_email'=>Helper::Post('account_email'),
                'account_pass'=>hash('sha256', Helper::Post('account_pass')),
                'account_ctime'=>date('Y-m-d H:i:s', time())
        ];
            $is=$db->TInsertIFNot('accounts',$data," WHERE account_login=? OR account_email=?",[Helper::Post('account_email'),Helper::Post('account_email')]);
            if($is){
                $this->ViewData('error', 200);
                $this->ViewData('request', json_encode($data));
            } else {
                $this->ViewData('error', 405);
                $this->ViewData('request', json_encode(['message'=>'error']));
            }
        ...
        $this->ViewData('scope', "\"none\"");

        $this->ViewData('expires', "");
