...
{   
    ...
    $db->Connect(DBTYPE, DBNAME, DBUSER, DBPASS, DBHOST);
        ...
        if ($user && isset($user[0])) {
            ...
            $auth = ['*'];
            $check_token = $db->TSelect('accounts_token', $auth, 'WHERE user_id=? AND client_id=?', [$user[0]['id'],$appid]);
            ...
            if (!$check_token || (strtotime($expires) < time())) {
                ...
                $set_token = $db->TInsertUpdate('accounts_token', $tu,' WHERE user_id=? AND client_id=?', [$user[0]['id'],$appid]);

                $userid = $user[0]['id'];
                $this->ViewData('token','?access_token='.$userid.','.$token);
                header('location: ?access_token='.$userid.','.$token);
            } else {
                $userid = $user[0]['id'];
