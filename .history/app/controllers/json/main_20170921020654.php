...
class Main extends \library\Core\Controller
{
    ...
    //Inc('test/views/test/view','.html');  
        ...
        */
        ...
        $auth->request();
        //$request = $check_token;
        $this->ViewData('request', decode(sha1($appid),encode(sha1($appid),json_encode($auth->request))));
        ...
        $this->ViewData('response', decode(sha1($appid),encode(sha1($appid),'<a href="?logout=true">logout</a>')));
