...
{   
    ...
    $db->Connect(DBTYPE, DBNAME, DBUSER, DBPASS, DBHOST);
       ...
       //echo "SID: ".SID."<br>session_id(): ".session_id()."<br>COOKIE: ".$_COOKIE["PHPSESSID"];
        ...
        $user = $db->TSelect('accounts', $scope, 'WHERE ' . $scope[1] . '=? AND ' . $scope[3] . '=?', [Helper::Post($scope[1]),Helper::Post($scope[3])]);
        ...
        if ($user && isset($user[0])) {
            ...
            if (!$check_token || (strtotime($expires) > time())) {
                ...
                //
                $this->ViewData('token','?access_token='.$token);
                header('location: ?access_token='.$token);
            }
        }
