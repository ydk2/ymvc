...
class Main extends \library\Core\Controller
{
    ...
    //Inc('test/views/test/view','.html');  
        ...
        */
        ...
        $userid = Helper::Post('userid');
        $token = Helper::Get('access_token');
        $appid = $sessionid;
        list($user_id,$access_token) = explode(',',$token);

        $check_token = $db->TSelect('accounts_token', $auth, 'WHERE client_id=? AND user_id=? AND access_token=?', [$appid,$user_id,$access_token]);
