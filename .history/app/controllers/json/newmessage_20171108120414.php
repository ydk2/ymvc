...
class NewMessage extends \library\Core\Controller
{
    ...
    private function update(){
        ...
        $share = json_decode($this->auth->request['account_share']);
        $user = array();

        foreach ($share as $value) {
            $user[$value]=$this->auth->request['account_'.$value];
        }
        $data = array(
            'appid'=>$appid,
            'from_id'=>$from_id,
        ...
        $new = $this->db->TinsertUpdate('accounts_msg',$data," WHERE msgid=? AND from_id=? AND msgid=?",[$msgid,$from_id,$msgid]);
