...
class Langs extends \library\Core\Controller
{
    ...
    public function Run()
    {
        ...
        if(Helper::Get('get')!=''){
            $filename = strtolower(str_replace(array('\\', '/'), DIRECTORY_SEPARATOR, ROOT."/languages/".Helper::Get('get')));
            if (file_exists($filename) && is_file($filename)) {
            $content = file_get_contents($filename);
            $this->ViewData('content',$content);
            } else {
                $this->ViewData('content','{"error":404}');
                header('Last-Modified: '.gmdate('D, d M Y H:i:s', filemtime(__FILE__)).' GMT', true, 404);
            }
        } else {
            $this->ViewData('content','{"error":400}');
            header('Last-Modified: '.gmdate('D, d M Y H:i:s', filemtime(__FILE__)).' GMT', true, 400);
        }
        if (!$this->error) $this->error = 0;
        //$this->ViewData('error', $this->error);
        if ($this->error) {
