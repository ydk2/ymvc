...
class Time extends \library\Core\Controller
{
    ...
    public function Run()
    {
        ...
        if ($auth->error == 200) {
            ...
            if($d <= 58){
                $auth->regenerate();
                $this->ViewData('data', \LZCompressor\LZString::compress('{"time":"'.$now.'","expires":"'.$d.'","token":"'.$auth->token.'"}'));
            } else {
                $this->ViewData('data', \LZCompressor\LZString::compress('{"time":"'.$now.'","expires":"'.$d.'"}'));
            }
