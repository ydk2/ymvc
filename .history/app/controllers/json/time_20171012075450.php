...
class Time extends \library\Core\Controller
{
    ...
    public function Run()
    {
        ...
        if ($auth->error == 200) {
            ...
            if($d <= 58){
                $auth->regenerate();
                $this->ViewData('data', \LZCompressor\LZString::compress(json_decode(json_encode('{"error":200,"token":"'.$auth->token.'"}'))));
            } else {
                $this->ViewData('data', \LZCompressor\LZString::compress('{"error":200,"time":"'.$now.'","expires":"'.$d.'"}'));
            }
            
        } else {
            $this->ViewData('event', 'message');
            $this->ViewData('data', \LZCompressor\LZString::compress('{"error":401}'));
