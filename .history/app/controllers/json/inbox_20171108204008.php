...
class Inbox extends \library\Core\Controller
{
    ...
    private function update()
    {
        ...
        $msg = $this->db->TSelect('accounts_msg a', ['a.*'], " WHERE NOT EXISTS (SELECT b.msgid,b.for_id FROM accounts_msg b WHERE b.for_id=? AND b.msgid=a.msgid) AND a.for_id<>? AND a.from_id<>? AND a.status=0 ORDER BY a.mtime DESC", [$for_id,$for_id, $from_id]);
        ...
        if (empty($msg)) {
            ...
            $this->ViewData('response', json_encode($readed));
        }
