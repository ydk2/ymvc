...
class SystemData extends DBConnect {
    ...
    public function add_item($table,$name,$value,$idx,$grpx) {
        try {
            $a = $this -> db -> query("SELECT title, link, lang, grpx FROM ".DBPREFIX.$table." WHERE link='$item_link' AND grpx='$grpx' AND lang='".$this->lang_menu."'");
            $check = $a -> fetchColumn();
            if ($check == TRUE) {
                return 1069;
            } else {
                $i = count($this -> get_menu($grpx)) + 1;
                $add = $this -> db -> prepare("INSERT INTO ".DBPREFIX.$table." (pos, title, parent, link, lang, grpx) VALUES (?,?,?,?,?,?)");
