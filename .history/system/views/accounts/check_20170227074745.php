...
<?php switch ($key) {
  case 'account_name':?>
    ...
    <?=(helper::validate($value,'alphanum',3,40))?'<b class="text-success">'.intl::_('ok').'</b>':'<b class="text-danger">'.intl::_('popraw').'</b>';?>
    <?php break;
  case 'account_email':?>
...
<?php switch ($key) {
  ...
  case 'account_ctime':
  case 'account_mtime':?>
    <span><?=date('d-m-Y',$value)?> <?=(helper::validate(date('d-m-Y',$value),'date'))?intl::_('ok'):intl::_('popraw')?></span>
    <?php break;
  case 'account_adnotation':?>
    <span><?=$value?></span>
    <?php break;
