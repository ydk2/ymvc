...
if($pagination!=null){
  $pagination->limit = 5;
  $pagination->items = $this->files;
  $list = $pagination->paginate_list($this->files);
}
?>
<div class="row">
...
<a class="btn btn-warning" href="<?=$this->data->link.'&path='.base64_encode(dirname(base64_decode(helper::get('path')))).helper::Session('back');?>"><?=intl::_('Wróć')?></a>
<?php } ?>
...
<a class="btn btn-link lead" href="<?=$this->data->link.'&path='.base64_encode($path);?>"><?=$file['name'];?>&#160;</a>
...
if($pagination!=null){
  ...
  $back = (helper::get($pagination->getstr)=="")?"":'&'.$pagination->getstr.'='.helper::get($pagination->getstr);
  helper::Session_Set('back',$back);
}
?>
