  ...
  <div class="row">
	<ul class="breadcrumb">
    <li><strong><?=intl::_('Pokaż w kolumnach')?></strong></li>
  ...
  <div class="row">
      <h3><?=intl::_('Edytuj Układ')?></h3>
      <form class="form" role="form" action="<?=$this->data->link?>&action=update" method="post">
      <div class="col-sm-12">
      <button class="btn btn-success" type="submit"><?=intl::_('Zmień')?></button>
      <a class="btn btn-info" href="<?=$this->data->link?>"><?=intl::_('Odśwież')?></a>
    ...
    ?>
        <a class="btn btn-success" href="<?=$this->link?>&group=<?=$entry['name']?>" ><?=intl::_('Edytuj')?></a>
    <?php }
    if(in_array($entry['module'],$this->menus)){
    ?>
        <a class="btn btn-success" href="?manage<?=S?>manage=manage<?=S?>menus&group=<?=$this->group?>" ><?=intl::_('Edytuj menu')?></a>
    <?php } if($entry['attr']!="" && ($entry['module']!="menu" || $entry['module']!="elements-menu")) { ?>
        <a class="btn btn-success" href="?manage<?=S?>manage=manage<?=S?>menus&group=<?=$entry['attr']?>" ><?=intl::_('Edytuj menu')?></a>
    <?php } ?>
            </div>
      <?php endforeach;?>
      </div>
  ...
  <div class="col-sm-offset-2 col-sm-8">
    <div class="well">
      ...
      <a href="<?=$this->data->link?>" class="btn btn-info btn-large"><?=intl::_('OK')?></a>
    </div>
  </div>
  <?php endif;?>

  <div class="col-sm-offset-2 col-sm-8 well">
      <h3><?=intl::_('Nowy układ')?></h3>
       ...
       <button name="add" type="submit" class="btn btn-block btn-success"><?=intl::_('Dodaj')?></button>
       </form>
  </div>
