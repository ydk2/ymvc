  ...
  <div class="row">
      ...
      <a class="btn btn-info pull-right" href="<?=$this->data->link?>">reload</a>
      <hr>
      </div>
      <?php foreach($this->items as $id=>$entry):?>
            ...
            <a class="btn btn-danger" href="<?=$this->data->link?>&action=delete&item=<?=$entry['id']?>">Remove</a>    $special = array('layout','route');
    <?php
    if(in_array($value['module'],$this->special)){
    ?>
        <a class="btn btn-success" href="<?=$this->link?>&group=<?=$entry['name']?>" >Edytuj</a>
    <?php }
    if(in_array($value['module'],$this->menus)){
      ?>
        <a class="btn btn-success" href="?manage<?=S?>menus&group=<?=$this->group?>" >Edytuj menu</a>
   <?php } ?>
            </div>
      <?php endforeach;?>
      </form>
