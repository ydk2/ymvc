  ...
  <?php if(!Helper::post('items')):?>
    <?php if(!empty($this->menuitems)):?>
      ...
      <?php foreach($this->menuitems as $id=>$entry):?>
            <div>
            <select class="form-control col-sm-1" name="items[<?=$id?>][pos]">
            <?php $i=1; foreach ($this->menuitems as $pos) : ?>
            <?php $selected = ($entry['pos']===$i)?' selected="selected"':''; ?>
              <option value="<?=$i?>"<?=$selected?>><?=$i?></option>
            <?php $i++; endforeach;?>
            </select>
            <input class="form-control col-sm-4" type="text" name="items[<?=$id?>][title]" value="<?=$entry['title']?>">
            <input class="form-control col-sm-4" type="text" name="items[<?=$id?>][link]" value="<?=$entry['link']?>">

            <select class="form-control col-sm-4" name="items[<?=$id?>][parent]">
