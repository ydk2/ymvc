  ...
  <?php if(!Helper::post('items')):?>
    <?php if(!empty($this->menuitems)):?>
      ...
      <?php foreach($this->menuitems as $id=>$entry):?>
            ...
            <input class="col-sm-4" type="text" name="items[<?=$id?>][title]" value="<?=$entry['title']?>">
            <input class="col-sm-4" type="text" name="items[<?=$id?>][link]" value="<?=$entry['link']?>">

            <select class="col-sm-4" name="items[<?=$id?>][parent]">
