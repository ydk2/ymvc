...
class Pagination extends Render {
	...
	public function Run(){
		...
		$start = (helper::get('p')=='')?1:intval(helper::get('p'));
		$limit = $this->limit;
		$this->paginate_count($this->items,$limit);
	}

    public function paginate_list($array=array(),$offset=1,$limit=10){
		$lenght = count($array);
		$end = ceil($lenght/$limit);
		//(helper::get('p')=='')?1:intval(helper::get('p'));
		return array_slice($array,($offset-1)*$limit,$limit);
    }

    public function paginate_count($array=array(),$limit=10){
		$lenght = count($array);
		$this->lenght = ceil($lenght/$limit);
    }
