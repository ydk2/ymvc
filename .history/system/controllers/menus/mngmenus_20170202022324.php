...
class MNGMenus extends PHPRender {
    ...
    private function Save(){
        if(Helper::get('action')=='add' && isset($_POST['add'])){
            ...
            $save = file_put_contents(ROOT.STORE.'layouts.data',serialize($this->datalist));
            if($save){
	...
	function edit_menu($data, $parent = '') {
		...
		foreach ($data as $item) {
            if($item['group']===$this->group){
				...
				$tree .= '<td><a class="btn btn-danger" href="' . HOST_URL . '?admin'.S.'mngmenus&action=delete_item&item=' . $item['id'] . '&data='.$this->group.'">Delete entry</a></td>';
