...
class MNGMenus extends PHPRender {
	...
	public function Init() {
		...
		$this->access_groups = array('menus','editor');
    ...
    public function Run(){
        ...
        $this->SetParameter('','action',HOST_URL.'?menus'.S.'mngmenuss&group='.$this->group.'');
        $this->SetParameter('','addgroup',HOST_URL.'?menus'.S.'mngmenuss');
        $this->SetParameter('','addgrouphidden','menus'.S.'mngmenuss');
        ...
        if(Helper::get('action')){
            ...
            //$this->Save();
            $this->data->message->addChild('link', HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$this->group.'');
        } else {
            if(!empty($this->datalist)){
               // $this->ViewData('menuss', '');
              //  $this->inColumn();
    ...
    private function Save(){
        if(Helper::get('action')=='add' && isset($_POST['add'])){
            ...
            $save = file_put_contents(ROOT.STORE.'menuss.data',serialize($this->datalist));
            if($save){
                    $this->data->message->header='Udane';
                    $this->data->message->text= 'Operacja zakończona pomyślnie';
                } else {
                    $this->data->message->header= 'Nie Udane';
                    $this->data->message->text= 'Operacja zakończona błędem';
                }
	...
	function menu($data) {
		...
		$i = 1;
		foreach ($data as $item) {
				$tree .= '<a class="list-group-item" href="'.HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$item.'">'.$item.'</a>' . PHP_EOL;
			}
		$tree .= "</div>";
		return $tree;
	}
    ...
    public function inColumn(){
        ...
        $showsort .= '<li><a href="'.HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$this->group.'&amp;showas=one">jednej</a></li>';
        $showsort .= '<li><a href="'.HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$this->group.'&amp;showas=two">dwóch</a></li>';
        $showsort .= '<li><a href="'.HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$this->group.'&amp;showas=three">trzech</a></li>';
        $showsort .= '</ul></div>';
        $this->ViewData('header', '<h3>Manage menuss</h3>'.$showsort);
        switch ($showed) {
            case 'three':
                $showas = 'col-sm-4';
                break;
    ...
    public function menus($enabled,$disabled){
        ...
        if(!empty($this->datalist)){
            ...
            if($value['group']==$this->group && $yes){
                ...
                if($contents!=""){
                    $col = $this->data->menuss->addChild('items', $contents);
    ...
    public function menus_values($value){
        ...
        if(in_array($value['module'],$special)){
            $append .= '<a class="btn btn-success" href="'.HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$value['name'].'" >Edytuj</a>';
        }
        ...
        $contents .= $sbtngrp.$append.'<a class="btn btn-danger" href="'.HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$value['group'].'&amp;action=delete&amp;item='.$value['id'].'">Usuń</a>'.$ebtngrp.$egroup;
    ...
    public function add_menus_item(){
        ...
        $sgroup ='<div class="input-group">';
        $egroup ='</div>'."\n";
        $sform = '<form action="'.HOST_URL.'?menus'.S.'mngmenuss&amp;group='.$this->group.'&amp;action=add" method="post">';
        $eform = '</form>';
        $ipos = 1;
        ...
        $pos = $ipos;
        $idx = $this->model->Get_Free_Idx('menuss','menus');
