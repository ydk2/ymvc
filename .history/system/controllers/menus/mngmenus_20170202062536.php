...
class MNGMenus extends PHPRender {
    ...
    public function Run(){
        ...
        //$this->datalist[1]=array('id'=>1,'pos'=>1,'title'=>'none','link'=>'null','parent'=>'','group'=>'main');
	...
	function edit_menu($data, $parent = '') {
		...
		foreach ($data as $item) {
            if($item['group']===$this->group){
				...
				$tree .= '<input class="form-control" type="hidden" id="ids" name="item[' . $i . '][id]" value="' . $i. '"' . ">";
				$tree .= '<input class="form-control" type="hidden" id="ids" name="item[' . $i . '][group]" value="' . $this->group. '"' . ">";
				...
				$tree .= '<td><a class="btn btn-danger" href="' . HOST_URL . '?menus'.S.'mngmenus&action=delete&item=' . $i . '&group='.$this->group.'">Delete entry</a></td>';
