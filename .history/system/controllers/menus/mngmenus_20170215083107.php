...
class MNGMenus extends PHPRender {
    ...
    public function Run(){
        ...
        **/
        ...
        for ($i=1; $i<count($updateditems)+1; $i++) {
            $updateditems[$i]['pos']=$i;
        }
        reset($this->datalist);
        while (list($a, $value) = each($this->datalist)) {
            reset($updateditems);
            while (list($b, $updated) = each($updateditems)) {
                if($value['id']==$updated['id']){
                    $this->out[$value['id']]=$updated;
                } else {
                    $this->out[$value['id']]=$value;
        ...
        if($updateditems){
            $save = file_put_contents(ROOT.STORE.'menus.data',serialize($updateditems));
        }
        //return true;
        /**
