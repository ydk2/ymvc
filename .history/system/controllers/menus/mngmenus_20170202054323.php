...
class MNGMenus extends PHPRender {
    ...
    public function Run(){
        ...
        if(Helper::get('action')){
            //$this->Save();
           $this->ViewData('link', HOST_URL.'?menus'.S.'mngmenus&group='.$this->group.'');
        } else {
    ...
    private function Save(){
        if(Helper::get('action')=='add' && isset($_POST['add'])){
            ...
            foreach ($this->datalist as $pos => $val) {
                ...
                if($this->datalist[$freekey] == $frompost[$freekey]){
                    $this->ViewData('header', 'Udane');
                    $this->ViewData('text', 'Operacja zakończona pomyślnie');
                } else {
                    $this->ViewData('header', 'Nie Udane');
                    $this->ViewData('header', 'Operacja zakończona błędem');
                }

            } else {
                $this->ViewData('header',  'Uwaga!!!');
                $this->ViewData('text',  'Pola nazwy i modułu nie mogą być puste');
            }
        ...
        if(Helper::get('action')=='update' && isset($_POST['update'])){
            ...
            if(!$save){
                    $this->ViewData('header', 'Nie Udane');
                    $this->ViewData('text', 'Operacja zakończona niepomyślnie');
                }
        }
        if(Helper::get('action')=='delete' && Helper::get('item')){
            ...
            if(!$save){
                    $this->ViewData('header', 'Nie Udane');
                    $this->ViewData('text', 'Operacja zakończona błędem');
                }
        }
        $this->ViewData('header', 'Błąd!!!');
        $this->ViewData('header',  'Operacja Nie Istnieje');
    }
