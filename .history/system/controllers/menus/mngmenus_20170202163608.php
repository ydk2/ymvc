...
class MNGMenus extends PHPRender {
    ...
    public function Run(){
        ...
        $this->datalist[1]=array('id'=>4,'pos'=>4,'title'=>'none4','link'=>'null','parent'=>'','group'=>'main');
        $this->datalist[2]=array('id'=>5,'pos'=>5,'title'=>'none5','link'=>'null','parent'=>'','group'=>'main');
        $this->datalist[3]=array('id'=>6,'pos'=>6,'title'=>'none6','link'=>'null','parent'=>'','group'=>'main');
	...
	function edit_menu($data, $parent = '') {
		...
		foreach ($data as $item) {
            if($item['group']===$this->group){
				$tree .= '<tr>';
				$tree .= "<td>" . $this -> change_pos($data, $item['id'], $i) . "</td>";
