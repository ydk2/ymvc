...
class Layouts extends PHPRender {
    ...
    public function Run(){
        ...
        $this->datalist=$this->model->getData('layout_data');
        $this->model->splitData($this->datalist,$this->group,'group');
        $this->items = $this->model->items;
        $this->others = $this->model->others;
        $this->group_list=$this->group_list=$this->model->filter_list($this->datalist,'group');
    ...
    private function Save($updateditems=array()){
        ...
        $this->data->text=$this->dump($this->model->joinData($fixedpos,$this->others));
        return false;
        if(!$this->model->setData('layout_data',$this->model->joinData($fixedpos,$this->others))){
            $this->data->header= 'Nie Udane';
            $this->data->text= 'Operacja zakończona błędem';
        } else {
