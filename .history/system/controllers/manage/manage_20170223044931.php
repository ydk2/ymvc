...
class Manage extends PHPRender {
    ...
    public function Init() {
        ...
        */
        ...
        $this->inuse = staticCache::getCache(Config::$data['inuse']);
        if(!$this->inuse){
            $this->inuse = array();
        }

        if(!in_array(helper::session('token'),$this->inuse) && !array_key_exists($this->name,$this->inuse)){
        //var_dump($this->inuse);
        $this->inuse[$this->name]=helper::session('token');
        staticCache::setCache(Config::$data['inuse'],$this->inuse);
        } else {
            $this->data->link_yes=$this->link."&answer=yes";
            $this->data->link_no=$this->link."&answer=no";
            $this->data->header=intl::_("używane");
            $this->data->text=intl::_("używane przez").' '.$this->inuse[$this->name];
