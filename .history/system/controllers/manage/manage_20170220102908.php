...
class Manage extends PHPRender {
    ...
    public function Run(){
        switch ($this->view) {
            ...
            case SYS.V."manage".DS."modules" || SYS.V."manage".DS."modules".DS."list" || SYS.V."manage".DS."modules".DS.'edit':
    ...
    public function modules(){
        ...
        if(Helper::get('action')){
            ...
            $this->files=FileUtils::inDir(ROOT.$this->app.DS.C.Helper::get('path').'/*'); //+FileUtils::inDir(ROOT.APP.C.Helper::get('path').'/*');
        }
    ...
    private function Actionmodules(){
        ...
        if(Helper::post('items')){
           // $this->setview(SYS.V . "manage".S."modules".S.'edit');
           $this->postvals = $this->dump(helper::post('items'));
        if(!$this->model->setData(Config::$data['saved_data'],$this->model->joinData(helper::post('items'),$this->others))){
            $this->data->header= 'Nie Udane';
            $this->data->text= 'Operacja zakończona błędem';
        } else {
