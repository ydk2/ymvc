...
class Modules extends PHPRender {
    ...
    public function nav($data, $parent = '') {
        ...
        foreach ($data as $item) {
            if ($item['parent'] === $parent) {
                $tree[$item['id']]['childs'] = call_user_func_array(array($this, __FUNCTION__), array($data,strval($item['id'])));
                $tree[$item['id']]['items'] = $item;
            }
        }
        return $tree;
    }
    ...
    function navbar_menu($array,$parent = '',$parents = array()){
