...
class Layouts extends PHPRender {
    ...
    public function inColumn(){
        ...
        $this->columns=array();
        $this->columns[0]=array('Jednej',$this->data->link.'&cols=1');
        $this->columns[1]=array('Dwóch',$this->data->link.'&cols=2');
        $this->columns[2]=array('Trzech',$this->data->link.'&cols=3');
        switch ($showed) {
            case '3':
                $cols = 'col-sm-4';
                break;
    ...
    if(!empty($this->datalist)){
        ...
        if($value['group']==$this->group && $yes){
            ...
            if($contents!=""){
                ...
                if(isset($value['class'])) $col->addAttribute('class', $this->cols.' well');
            }
            $contents = NULL;
            $col = NULL;
        }
