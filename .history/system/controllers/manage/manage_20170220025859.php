...
class Manage extends PHPRender {
    ...
    public function choose(){
        $this->data->link = HOST_URL.'?manage'.S.'manage=manage'.S.'manage&group='.$this->group.'';
        $this->link = HOST_URL.'?manage'.S.'manage=manage'.S.'manage';
        $this->datalist=$this->model->getData(Config::$data['layout_data']);
    ...
    public function Run(){
        echo $this->view;
        switch ($this->view) {
            case SYS.V . "manage".S."manage":
                $this->choose();
                break;
            case SYS.V . "manage".S."layouts":
                $this->layout();
                break;
    ...
    private function SaveLayout($updateditems=array()){

        $fixedpos = $this->model->fixby($updateditems,'pos');
        if(!$this->model->setData(Config::$data['layout_data'],$this->model->joinData($fixedpos,$this->others))){
            $this->data->header= 'Nie Udane';
            $this->data->text= 'Operacja zakończona błędem';
        } else {
