...
private function uvalidate($entry,$type='text',$min=3,$max=30){
    $this->msg = "";
    $this->subview = "";
    switch ($type) {
        case 'text':
        if (!isset($entry) && $entry=="" && (strlen($entry)<$min && strlen($entry)>$max)) {
            return intl::_('Pole nie może być puste');
        } break;
        case 'alphanum':
        if (!isset($entry) && $entry=="" && !ctype_alnum($entry) && (strlen($entry)<$min && strlen($entry)>$max)) {
            return intl::_('Niewłaściwy format');
        } break;
        case 'uri':
        if (!ctype_alnum($entry) && !filter_var($entry, FILTER_VALIDATE_URL)) {
            return intl::_('Niewłaściwy format WWW');
        } break;
        case 'email':
        if (!ctype_alnum($entry) && !filter_var($entry, FILTER_VALIDATE_EMAIL)) {
            return intl::_('Niewłaściwy format e-mail');
        } break;
        case 'date':
        $test = explode('-', $entry);
        if (!ctype_alnum($entry) || count($test)!=3 || !checkdate($test[0], $test[1], $test[2])) {
            return intl::_('Niewłaściwy format daty');
        } break;
        case 'pass':
        if (!isset($entry) && $entry=="" && !ctype_alnum($entry) && (strlen($entry)<$min && strlen($entry)>$max)) {
            return intl::_('Puste pole');
        } break;
        case 'num':
        if (!is_numeric($entry) && (strlen($entry)<$min && strlen($entry)>$max)) {
            return intl::_('To nie numer');
        } break;
        default:
        if (!isset($entry) && $entry=="" && (strlen($entry)<$min && strlen($entry)>$max)) {
            return intl::_('Pole nie może być puste');
