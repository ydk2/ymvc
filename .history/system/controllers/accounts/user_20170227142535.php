...
public function uchecknew(){
    ...
    if(isset($this->post) && !empty($this->post)){
        ...
        if($this->utest == count($this->required) && helper::session('account_new')){
            $this->usavenew();
        }
}
...
private function usavenew(){
    ...
    if($chk){
        ...
        $this->user=$this->model->Select($table,array('id','account_login'),'where account_login=?',array(helper::session('account_new')['account_login']));
