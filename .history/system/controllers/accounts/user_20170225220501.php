...
* @Last Modified time: 2017-02-25 20:22:59
*/
...
public function utest(){
    ...
    foreach ($this->userdetails as $key => $value) {
        if(strpos($key, "account_") !== false && $value != ''){
            $savemaindata[$key] = $value;
            $savemaindata['account_born']=strtotime($this->userdetails['account_born']);
        }
    }
    //var_dump($savemaindata);
    ...
    if($chk){
        ...
        $this->data->link_no=$this->link."&answer=no";
        $this->header='Udane';
        $this->text='Operacja zakończona pomyślnie';
        $this->model->Release(Helper::Session('token'));
        $this->msg = $this->subView(SYS.V."elements-answer");
    } else {
        $this->data->link_yes=$this->link."&answer=yes";
        $this->data->link_no=$this->link."&answer=no";
        $this->header= 'Nie Udane';
        $this->text='Operacja zakończona błędem';
        $this->model->Rollback(Helper::Session('token'));
        $this->msg = $this->subView(SYS.V."elements-answer");
    }
    $this->subview=$this->dump($this->model->Query("SELECT * FROM ".$table.""));
}
