...
private function uvalidate($entry,$type='text',$min=3,$max=30){
    ...
    switch ($type) {
        ...
        if (!$doc->validate() || (strlen($entry)<$min && strlen($entry)>$max)) {
            return intl::_('To nie HTML');
        } break;
        case 'xml':
        $doc = new DOMDocument();
        $doc->loadXML($entry);
        if (!$doc->validate() || (strlen($entry)<$min && strlen($entry)>$max)) {
            return intl::_('To nie XML');
        } break;
        case 'text':
        default:
        ...
        if(strpos($key, "account_") !== false && $value != ''){
            switch ($key) {
                ...
                case 'account_born':
                    ...
                    $this->subview2 .= '<h2>'.$value.' '.$this->uvalidate($value,'date').'</h2>';
                    break;
                case 'account_email':
                    $this->subview2 .= '<h2>'.$value.' '.$this->uvalidate($value,'email').'</h2>';
                    break;
                case 'account_pass':
                    $this->subview2 .= '<h2>'.$value.' '.$this->uvalidate($value,'alphanum').'</h2>';
                    break;
                case 'account_adnotation':
                    $this->subview2 .= '<h2>'.$value.' '.$this->uvalidate($value,'text',20,100).'</h2>';
                    break;
