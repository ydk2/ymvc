...
**/
    ...
    public function Layouts2($group='main'){
		...
		if(!in_array($key,$disabled)){
			$content->layouts[] = array('pos' => $i++, 'name'=>'FromRoute_'.$key,'module'=>$key,'view'=>$route,'class'=>$content->attrclass,'attrid'=>'', 'users'=>'', 'group'=>$content->layout_group, 'mode'=>$content->mode);
		}
		}
                        $contents = ($content)? htmlspecialchars($content->View()):"";
                        if($contents!=""){
