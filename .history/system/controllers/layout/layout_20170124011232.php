...
class Layout extends XSLRender {
    ...
    public function Run(){
        ...
        //$aout[4][count($data)+1]['_view'] = 'four';
         ...
         $this->setValue($aout,4,'_view','cccc');
        //unset($aout[2][10]);
...
*/
    ...
    public function DeleteValue(&$data,$i,$name){
        foreach ($data as $index => $items) {
            if($i == $index){
            foreach ($items as $key => $value) {
                if(isset($value[$name])){
                    unset($data[$index][$key][key($value)]);
                break;
                }
            }
            }
    ...
    public function DeleteEntry(&$data,$i,$name){
        foreach ($data as $index => $items) {
            if($i == $index){
            foreach ($items as $key => $value) {
                if(isset($value[$name])){
                    ...
                    if(!in_array($value['name'],$disabled)){
		                ...
		                foreach ($_GET as $key => $router) {
                            if(in_array($mode.C.$key,$enabled) && !in_array($mode.C.$key,$disabled) && $this->ControllerExists($mode.C.$key)){
			                    $content->layouts[] = array('pos' => $pos++, 'name'=>'FromRoute_'.$key,'module'=>$key,'view'=>$router,'class'=>$value['class'],'attrid'=>'', 'users'=>'', 'group'=>$content->layout_group, 'mode'=>$value['mode']);
