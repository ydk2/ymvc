...
class Route extends XSLRender {
	...
	public function Run($model = NULL){
        ...
        if(!isset($this->layout_data)&&$this->layout_data==''){
          $this->layout_data=Config::$data['layout_data'];
        }
		echo ROOT.SYS.STORE.$this->model->layout_data;
		$items = json_decode(file_get_contents(ROOT.SYS.STORE.$this->layout_data),true);
		if (empty($items)){
		...
		if(!in_array($key,$this->model->disabled)){
			$this->model->layouts[] = array('pos' => $i++, 'name'=>'FromRoute_'.$key,'module'=>$key,'view'=>$value,'class'=>'col-sm-12','attrid'=>'', 'users'=>'', 'group'=>'route', 'mode'=>$this->mode);
		}
		}
		//$this->router();
		$this->contents();		
