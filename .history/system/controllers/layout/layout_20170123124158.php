            ...
            if($value['group']==$group && $yes && $value['group']!=$value['name']){
                ...
                if($value['module']=="layout" && $value['group']!=""){
                    ...
                    if(!in_array($value['name'],$disabled)){
                        ...
                        $content->model->enabled = $this->enabled;
		$i = 1;
		foreach ($_GET as $key => $value) {
		if(!in_array($key,$disabled)){
			$content->model->layouts[] = array('pos' => $i++, 'name'=>'FromRoute_'.$key,'module'=>$key,'view'=>$value,'class'=>$content->model->attrclass,'attrid'=>'', 'users'=>'', 'group'=>$content->model->layout_group, 'mode'=>$content->model->mode);
		}
		}
                        $contents = ($content)? htmlspecialchars($content->View()):"";
                        if($contents!=""){
