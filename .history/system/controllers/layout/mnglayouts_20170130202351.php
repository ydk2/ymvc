        ...
        if(Helper::get('action')=='add' && isset($_POST['add_item'])){
            ...
            if($frompost[$key]['name']!='' && $frompost[$key]['module']!=''){
                ...
                foreach ($data as $name => $items) {
                    $this->model->add_item('layouts',$items['name'],$items['value'],$items['idx'],'layout');
                }
            ...
            /**/
            $this->data->message->addChild('text', $this->dump($this->datalist));
        }
...
public function dump($value){
    ...
    public function Layouts($enabled,$disabled){
            reset($this->datalist);
        if(isset($this->datalist[key($this->datalist)]['pos'])){
    ...
    public function select($id,$pos,$name,$new=FALSE){
        ...
        for ($i=0; $i<count($this->datalist)+1; $i++) {
            if(isset($this->datalist[$i]['group']) && $this->datalist[$i]['group']==$this->group){
                $sel = ($pos == $i)?" selected='selected'":"";
                $options .= "<option value='".trim($i)."'".$sel.">".trim($i)."</option>";
                continue;
            }
        }
        if($new){
...
echo $value['pos'];
        ...
        $contents .= $sgroup.$this->select($value['id'],$value['pos'],'pos',true);
