...
**/
    ...
    public function Layouts2($group='main'){
		...
		if(!in_array($key,$disabled)){
			$this->model->layouts[] = array('pos' => $i++, 'name'=>'FromRoute_'.$key,'module'=>$key,'view'=>$value,'class'=>$this->model->attrclass,'attrid'=>'', 'users'=>'', 'group'=>$this->model->layout_group, 'mode'=>$this->model->mode);
		}
		}
                        $contents = ($content)? htmlspecialchars($content->View()):"";
                        if($contents!=""){
