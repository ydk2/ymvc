...
class MNGLayouts extends XSLRender {
    ...
    public function inColumn(){
        ...
        $this->columns->addChild('list');
        $this->columns->list->addChild('link',HOST_URL.'?layout'.S.'mnglayouts&amp;group='.$this->group.'&amp;showas=one');
        $this->columns->list->addChild('title','Jednej');
        $this->columns->addChild('list');
        $this->columns->list->addChild('link',HOST_URL.'?layout'.S.'mnglayouts&amp;group='.$this->group.'&amp;showas=two');
        $this->columns->list->addChild('title','Dwóch');
        switch ($showed) {
            case 'three':
                $showas = 'col-sm-4';
                break;
