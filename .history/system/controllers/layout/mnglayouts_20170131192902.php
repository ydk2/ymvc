...
class MNGLayouts extends XSLRender {
    ...
    public function inColumn(){
        ...
        $columns->list->addChild('link',HOST_URL.'?layout'.S.'mnglayouts&amp;group='.$this->group.'&amp;showas=one');
        $columns->list->addChild('title','Jednej');
        $columns->addChild('list');
        $columns->list->addChild('link',HOST_URL.'?layout'.S.'mnglayouts&amp;group='.$this->group.'&amp;showas=two');
        $columns->list->addChild('title','Dwóch');
        switch ($showed) {
            case 'three':
                $showas = 'col-sm-4';
                break;
