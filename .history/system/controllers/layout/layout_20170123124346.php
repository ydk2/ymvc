...
**/
    ...
    public function Layouts2($group='main'){
		...
		if(!in_array($key,$disabled)){
			$content->layouts[] = array('pos' => $i++, 'name'=>'FromRoute_'.$key,'module'=>$key,'view'=>$value,'class'=>$content->model->attrclass,'attrid'=>'', 'users'=>'', 'group'=>$content->model->layout_group, 'mode'=>$content->model->mode);
		}
		}
                        $contents = ($content)? htmlspecialchars($content->View()):"";
                        if($contents!=""){
