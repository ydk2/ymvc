...
public $mode;
	...
	public function onRun(){
		...
		if(isset($this->model->layout_parent)) $this->layout_parent = $this->model->layout_parent;
		$this->mode = (isset($this->model->mode) && $this->model->mode!="")?$this->model->mode:SYS;
		Config::$data['layouts']['current'] = $this->layout_parent;
		//$this->SetView(SYS.V.'index');
		$this->Layouts();
		
	}

	public function Layouts($parent='main'){
		$array = $this->layouts;
		$enabled = $this->enabled;
		$disabled = $this->disabled;
		$mode = $this->mode;
		$parent = $this->layout_parent;
		if(isset($array[0]['pos'])){
		$this->sksort($array,'pos');
		$check = array('pos', 'name','module','view','class','parent','attrid');
		$yes = TRUE;
		...
		foreach ($array as $value) {
			...
			if($value['parent']==$parent && $yes && $value['parent']!=$value['name']){
				if ($value['mode']=='sys') {
					$mode = SYS;
			...
			if($value['module']=="layout" && $value['parent']!=""){
			...
			if(!in_array($value['name'],$disabled)){
				...
				$content->model->layout_parent = $value['name'];
			...
			} elseif($value['module']=="route" && $value['parent']!="route"){
			...
			if(!in_array($value['name'],$disabled)){
				...
				$content->model->layout_parent = "route";
				$contents = ($content)? htmlspecialchars($content->View()):"";
				if($contents!=""){
				$col = $this->data->layout->addChild('views', $contents);
			...
			} elseif($value['module']=="section" && $value['parent']!=""){
	...
	public function Sections($array,$disabled,$mode=SYS,$parent=''){
		if(isset($array[0]['pos'])){
		$this->sksort($array,'pos');
		$check = array('pos', 'name','module','view','class','model','parent','attrid','users');
		$yes = TRUE;
		$this->SetView(SYS.V.'layout'.S.'sections');
		$this->ViewData('layout', '');
		foreach ($array as $value) {
			...
			if($value['parent']==$parent || $value['parent']=="" && $yes){
