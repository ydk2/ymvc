...
class Administration extends PHPRender {
    ...
    public function Init() {
        ...
        */
        ...
        $this->subitems = $this->model->itemsData($this->datalist,$this->current_group,'group');
    ...
    public function Run(){
        $chk = 0;
       // $this->model->createTableRotate('mainsettings','main');
        $defauclt[1]=array(
            'theme'=>'empty',
            'menu'=>'default',
            'layout'=>'any',
        ...
        $default[2]=array(
            '?admin'=>'Ustawienia Główne',
            '?admin-mngaccounts'=>'Zarządzaj Profilami',
            '?layout-mnglayouts'=>'Zarządzaj Rozkładem Strony',
            '?admin-mngmenus'=>'Zarządzaj Menu',
            '?test-test'=>'Test',
        );
        $testquery=array(
            'name'=>'empty1',
            'email'=>'default2@none.to',
            'password'=>sha1('empty1'),
        );
        $query = "";
        $this->model->dobegin(Helper::Session('token'));
        $chk=$this->model->Insert('users',$testquery);
        $chk=$this->model->doCommit(Helper::Session('token'));
            if($chk){
                $this->header='Udane';
                $this->message='Operacja zakończona pomyślnie';
                //$this->model->Release(Helper::Session('token'));
            } else {
                $this->header= 'Nie Udane';
                $this->message='Operacja zakończona błędem';
                $this->model->doRollback(Helper::Session('token'));
            }

      //  var_dump($this->model->reverseNoId($default,'main'));
        //$this->model->Begin(Helper::Session('token'));
            //foreach ($default as $key => $value) {
            //foreach ($value as $name=>$entry) {
            //    $chk+=$this->model->update_item('mainsettings',$name,$entry,$key,'main');
            //}
            //}
            /* *
            $this->model->doAddItems($default,'mainsettings','main');

        $chk=$this->model->Commit(Helper::Session('token'));
                if($chk==count($default[2])+1){
                    $this->header='Udane';
                    $this->message='Operacja zakończona pomyślnie';
                    $this->model->Release(Helper::Session('token'));
                } else {
            ...
            $datalist=$this->model->search_entries('mainsettings','main');
            $this->datalist = $datalist[2];
            * */
