    ...
    public function Run(){
       ...
       // $this->model->createTableRotate('mainsettings','main');
        ...
        $testquery=array(
            ...
            'password'=>sha1('empty2'),
        );
        $query = "";
        $query .= $this->model->preBegin(Helper::Session('token'));
        $query .= $this->model->preInsert('users',$testquery);
        $query .= $this->model->preCommit(Helper::Session('token'));
        echo $query;
/*
            if($chk){
                $this->header='Udane';
