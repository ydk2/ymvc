...
class Administration extends PHPRender {
    ...
    public function Run(){
       ...
       // $this->model->createTableRotate('mainsettings','main');
        ...
        $testquery=array(
            'settingsname'=>'menu_data',
            'settingsentry'=>serialize($this->datalist),
            'settingstitle'=>'Menu entries',
        );
        $entries = array(
            "settingsname varchar(32) not null unique",
            "settingsentry TEXT ",
            "settingstitle varchar(42) default ''"
        );
        $this->model->createTable('systemdata',$entries);
        $this->model->Begin(Helper::Session('token'));
        $chk=$this->model->Insert('systemdata',$testquery);
