...
$render = new mainController('default');

$header = $render->newController("Theme/Controllers/v52/Header",'default');
//$header->Show();

echo $render->View('test/views'.$theme.'/core/one','.html');
if($render->error){
            $render->ViewData('header','Error');
            $render->ViewData('message','Error throwed with success '.$render->error);
            $render->throwError($this->View('test/views/'.$theme.'/core/error','.html'));
        }

$footer = $render->newController("Theme/Controllers/v52/Footer",'default');
