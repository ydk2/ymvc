...
class mainController
{
    ...
    public function __construct($theme = NULL)
    {
        ...
        $spath = str_replace(strtolower(ROOT), '', strtolower($this->path));
        $this->strip = explode(DIRECTORY_SEPARATOR, $spath);
        if(!is_array($this->strip)){
            $this->strip = explode("\\", $spath);
        }
        $vpath = array();
    ...
    public final function newController($controller)
    {
        if (is_object($controller)) {
            ...
            $ext = '.php';
            $controller = strtolower(str_replace(array('\\', '/'), DIRECTORY_SEPARATOR, $controller));
    ...
    public final function newClass($class)
    {
        if (is_object($class)) {
            ...
            $ext = '.php';
            $class = strtolower(str_replace(array('\\', '/'), DIRECTORY_SEPARATOR, $class));
