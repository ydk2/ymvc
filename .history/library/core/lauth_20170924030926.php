...
class lAuth
{
    ...
    public function authorize($login, $password, $force = FALSE)
    {
        $token = "";
        $request = FALSE;
        $user = $this->db->TSelect('accounts', $this->scope, "WHERE account_login=? OR account_email=? AND account_pass='".hash('sha256', $password)."'", array($login, $login));
    ...
    {
        $expires = date('Y-m-d H:i:s', (time() - 60));
        $deauthorize = $this->db->TUpdate('accounts_token',array('expires'=>$expires), 'WHERE user_id=?', [$this->userid]);
        if ($deauthorize) {
            $this->userid = NULL;
            $this->token = NULL;
            $this->request = NULL;
