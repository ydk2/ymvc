...
class Core
{
    ...
    public function __construct($theme = NULL)
    {
        ...
        $this->view = str_replace(strtolower(ROOT),'',$this->view);
    ...
    public final function Controller($controller)
    {
        if (is_object($controller)) {
            ...
            $ext = '.php';
            $controller = strtolower(str_replace(array('\\', '/'), DIRECTORY_SEPARATOR, ROOT . DIRECTORY_SEPARATOR . $view));
            if ($this->Inc($controller,$ext)) {
                $stack = explode(DIRECTORY_SEPARATOR, $controller);
                $end = end($stack);
                if (!class_exists($end)) return NULL;
                $a = func_get_args();
                if(!empty($a)) array_shift($a);
