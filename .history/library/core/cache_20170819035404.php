...
class Cache
{
    ...
    public function GetItemBy($items,$key,$by='id'){
		reset($items);
	    while (list($a, $value) = each($items)) {
            if($value[$by] == $key){
                return $value;
    ...
    public function group($cache,$filter='group'){
        if(!empty($cache)){
            $group = array();
    ...
    public function used($cache,$by,$filter='group'){
		$items=array();
        if(!empty($cache)){
            foreach ($cache as $entry) {
                if($entry[$filter]==$by){
    ...
    public function unused($cache,$by,$filter='group'){
		$items=array();
        if(!empty($cache)){
            foreach ($cache as $entry) {
                if($entry[$filter]!=$by){
