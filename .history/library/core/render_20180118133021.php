...
private function to_xml( $data, &$xml_data ) {
    foreach( $data as $key => $value ) {
        $isattrib = (substr($key,0,1)=='@')?TRUE:FALSE;
        if( is_numeric($key) ){
			$key = 'item'.$key;
        }
        if(is_array($value)) {
            ...
            if($isattrib==TRUE){
                //$subnode = $xml_data->addChild($key);
                $this->to_xml($value, $xml_data);
            }
        } else {
            if(!$isattrib) $xml_data->addChild($key,htmlspecialchars($value));
            if($isattrib===TRUE) $xml_data->addChild(substr($key,1,0),$value);
        }
	} 
}
