...
class DB {
    ...
    final public function LoadPlugin($plugin){
        if (is_object($plugin)) {
            ...
            $ext = '.php';
            $path= strtolower(str_replace(array('\\', '/'), DIRECTORY_SEPARATOR,  $plugin));
            $plugin = strtolower(str_replace(array('\\', '/'), '\\',  $plugin));
            if ($this->Inc($path.$ext)) {
               
                //if (!class_exists($plugin)) return NULL;
                $a = func_get_args();
                array_shift($a);
                $new = new \ReflectionClass($plugin);
        ...
        try {
            ...
            if ($engin == 'sqlite') {
                if($this->Inc('/library/core/plugins/'.$engin.".php")){
                    //$plugin = "\\Library\\Core\\Plugins\\$engin";
                    //$this->db = new $plugin($data);
 ...
 */
    public function Lock($name,$which='WRITE'){
        ...
        */
        try {
            $add = $this -> db -> exec($this->db->Sql(__FUNCTION__,$name,$which));
            return TRUE;
        } catch(Exception $e){
            if(defined('DBDEBUG'))
            var_dump($e);
    ...
    public function createTable($table,$columns,$addid=TRUE) {
        ...
        try {
            $chk = $this -> db -> exec($this->sql['createTable']);
            return TRUE;
        } catch(Exception $e){
            if(defined('DBDEBUG'))
            var_dump($e);
    ...
    public function dropTable($table) {
        ...
        try {
            $chk = $this -> db -> exec($this->sql['dropTable']);
            return TRUE;
        } catch(Exception $e){
            if(defined('DBDEBUG'))
            var_dump($e);
