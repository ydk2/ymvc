        ...
        if($view!=NULL) {
            ...
            if (file_exists($filename) && is_file($filename)) {
				...
				if($controller){
					...
					$xml = $this->to_xml2($runner->data);
				} else {
					$xml = $this->to_xml2($this->data);
				}
				echo $xml;
				$data = new \SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><data>'.$xml.'</data>', null, false);

                $proc = new \XSLTProcessor;
                $proc->registerPHPFunctions();
                $proc->importStylesheet($xsl);
				$proc->setParameter('', 'self', $this->name.'::Call');
...
private function to_xml2( $data, $node="" ) {
	$_data = ($node=="")?'':'<?xml version="1.0" encoding="UTF-8"?><'.$node.'>';
    foreach( $data as $key => $value ) {
        if( is_numeric($key) ){
			//if($key != 0) $key = $key;
		}
