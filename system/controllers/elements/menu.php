...
class Menu extends Render {
	...
	public function ReadDB(){
        ...
        $this->datalist=$this->model->storeRead("menu");
        $this->items = $this->model->itemsData($this->datalist,$this->groups,'group');
	}
    ...
    public function navmenu($data, $parent = '') {
        ...
        foreach ($data as $item) {
            if ($item['parent'] === $parent) {
                ...
                $tree .= '<a href="'.htmlspecialchars($item['link']).'">' .intl::_($item['title']). PHP_EOL;
        ...
        foreach($array as $element)
        {
            if($element['parent']==$parent)
            {
                if(in_array($element['id'],$parents))
                {
                    ...
                    $menu_html .= '<a href="'.htmlspecialchars($url).'" class="dropdown-toggle" data-toggle="dropdown" role="menu-item" aria-expanded="true">'.Intl::_($element['title']).' <span class="caret"></span></a>'.PHP_EOL;
                	$menu_html .= '<ul class="dropdown-menu sub-menu" role="menu-item">'.PHP_EOL;
					$menu_html .= '<li>'.PHP_EOL;
					$menu_html .= '<a href="'.htmlspecialchars($url).'">' . Intl::_($element['title']) . '</a>'.PHP_EOL;
					$menu_html .= '</li>'.PHP_EOL;
